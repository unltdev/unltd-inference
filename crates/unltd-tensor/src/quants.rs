...
pub fn quantize_q8_k(x: &[f32]) -> Vec<Q8KBlock> {
    assert_eq!(
        x.len() % QK_K,
        0,
        "quantize_q8_k: len no es múltiplo de {QK_K}"
    );
...
pub fn gemv_quant(
    ...
    let row_bytes = match dtype {
        ...
        crate::DType::Q4K | crate::DType::Q6K => {
            assert_eq!(
                dim_in % QK_K,
                0,
                "gemv_quant: dim_in no es múltiplo de {QK_K}"
            );
            dim_in / QK_K
                * match dtype {
    ...
    assert_eq!(
        w.len(),
        row_bytes * dim_out,
        "gemv_quant: len(w) != filas exactas"
    );
    let x = &x[..dim_in];
    for j in 0..dim_out {
...
pub fn gemv_quant_q8k(
    ...
    let row_bytes = match dtype {
        crate::DType::Q4K | crate::DType::Q6K => {
            assert_eq!(
                dim_in % QK_K,
                0,
                "gemv_quant_q8k: dim_in no es múltiplo de {QK_K}"
            );
            dim_in / QK_K
                * match dtype {
                    crate::DType::Q4K => BLOCK_Q4_K_BYTES,
    ...
    assert_eq!(
        w.len(),
        row_bytes * dim_out,
        "gemv_quant_q8k: len(w) != filas exactas"
    );
    let x = &x[..dim_in];
...
mod tests {
    ...
    fn dequantize_q4_k_matches_reference_formula_bitwise() {
        ...
        let scales: [u8; 12] = [
            0x1F, 0x2E, 0x3D, 0x4C, 0x5B, 0x6A, 0x79, 0x03, 0x92, 0x81, 0x70, 0x6F,
        ];
    ...
    const Q6K_B0: [u8; 210] = [
        ,
    ];

    const Q6K_B15: [u8; 210] = [
        ,
    ];

    const Q4K_B0: [u8; 144] = [
        ,
    ];

    const Q4K_B15: [u8; 144] = [
        ,
    ];
